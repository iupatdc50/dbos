...
class Bar128Helper
{
     ...
     * @param string $type (string) barcode type: A, B, C or empty for automatic switch (AUTO mode)
     ...
     * @throws \Exception
     */
    public static function barcode_c128($code, $type = '', $widthFactor = 2, $totalHeight = 40, $color = 'black')
    {
        ...
        switch (strtoupper($type)) {
            ...
            default: { // MODE AUTO
                ...
                if (isset($numseq[1]) AND ! empty($numseq[1])) {
                    ...
                    if ($end_offset < $len) {
                        $sequence = array_merge($sequence, get128ABsequence(substr($code, $end_offset)));
                    }
                } else {
                    // text code (non C mode)
                    $sequence = array_merge($sequence, get128ABsequence($code));
                }
        ...
        if (!isset($bararray['maxWidth'])) {
            $newBarcodeArray = [];
            $newBarcodeArray['code'] = $bararray['code'];
            $newBarcodeArray['maxWidth'] = $bararray['maxw'];
            $newBarcodeArray['maxHeight'] = $bararray['maxh'];
            $newBarcodeArray['bars'] = [];
            foreach ($bararray['bcode'] as $oldbar) {
                $newBar = [];
            ...
            $bararray = $newBarcodeArray;
        }

//        return $bararray;

        $html = '<div style="font-size:0;position:relative;width:' . ($bararray['maxWidth'] * $widthFactor) . 'px;height:' . ($totalHeight) . 'px;">' . "\n";
        $positionHorizontal = 143;
        foreach ($bararray['bars'] as $bar) {
            $barWidth = round(($bar['width'] * $widthFactor), 3);
            $barHeight = round(($bar['height'] * $totalHeight / $bararray['maxHeight']), 3);
            if ($bar['drawBar']) {
                $positionVertical = round(($bar['positionVertical'] * $totalHeight / $bararray['maxHeight']), 3);
                // draw a vertical bar
